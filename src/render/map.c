...
static void	draw_wall_stripe(t_game *g, t_ray *ray, t_tex tex, int x)
{
	...
	while (++y <= ray->d_end.y)
	{
		d = (y - g->spider.pitch) * 256 - HEIGHT * 128 + ray->line_height * 128;
...
static void	draw_floor_and_ceiling(t_game *g, t_ray *ray, int y, t_tex tex)
{
	int				x;
	t_pos			map;
	unsigned int	color;
	...
	while (++x < GAME_W)
	{
		...
		ray->src.x = (int)((ray->side_dist.x - map.x) * tex.width);
		ray->src.y = (int)((ray->side_dist.y - map.y) * tex.height);
...
void	render_map(t_game *g)
{
	...
	while (++y < HEIGHT)
	{
		...
		p = (y - HEIGHT / 2) - g->spider.pitch;
		if (p == 0)
			continue ;
		ray.row_distance = ray.view / fabs(p);
		tex = g->map_text[C];
		if (p > 0)
			tex = g->map_text[F];
		draw_floor_and_ceiling(g, &ray, y, tex);
	}
	render_walls(g);
}
